...
app.get('/api/admin/cleanup-gb-accounts', async (req, res) => {
    try {
        if (req.query.key !== process.env.ADMIN_CLEANUP_KEY) {
            return res.status(403).json({ error: 'Unauthorized' });
        }
        ...
        while (hasMore) {
            const { data, error } = await supabase
                .from('users')
                .select('id, username, power_balance')
                .range(page * 1000, (page + 1) * 1000 - 1);
        ...
        res.json({
            ...
            summary: {
                ...
                matched_and_deleted: toDelete.length,
                deleted_ids: toDelete.slice(0, 100)
            }
        });
    } catch (error) {
        logError('/api/admin/cleanup-gb-accounts', error);
        res.status(500).json({ error: error.message });
    }
});
